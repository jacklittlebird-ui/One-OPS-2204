...
BEGIN
  ...
  new_handling := NULLIF(NEW.skd_type, '');
  ...
  IF new_handling IS NOT NULL THEN
    BEGIN
      UPDATE public.service_reports s
      SET
        aircraft_type = COALESCE(NULLIF(NEW.aircraft_type, ''), s.aircraft_type),
        handling_type = new_handling::handling_type
      WHERE TRIM(LOWER(s.flight_no)) = TRIM(LOWER(NEW.flight_no))
        AND ref_date IS NOT NULL
        AND (s.arrival_date::text = ref_date OR s.departure_date::text = ref_date);
    EXCEPTION WHEN invalid_text_representation OR undefined_object THEN
      -- skd_type value isn't a valid handling_type enum member — just skip the service_reports sync.
      NULL;
    END;
  ELSE
    UPDATE public.service_reports s
    SET aircraft_type = COALESCE(NULLIF(NEW.aircraft_type, ''), s.aircraft_type)
    WHERE TRIM(LOWER(s.flight_no)) = TRIM(LOWER(NEW.flight_no))
      AND ref_date IS NOT NULL
      AND (s.arrival_date::text = ref_date OR s.departure_date::text = ref_date);
  END IF;
