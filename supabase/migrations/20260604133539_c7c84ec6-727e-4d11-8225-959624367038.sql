...
BEGIN
  ...
  SET
    ...
    arrival_date = COALESCE(NEW.flight_date, arrival_date),
    updated_at = now()
...
DROP TRIGGER IF EXISTS sync_security_dispatch_to_flight_schedule_trigger ON public.dispatch_assignments;

CREATE TRIGGER sync_security_dispatch_to_flight_schedule_trigger
AFTER INSERT OR UPDATE OF task_sheet_data, service_type, flight_date
ON public.dispatch_assignments
FOR EACH ROW
EXECUTE FUNCTION public.sync_security_dispatch_to_flight_schedule();