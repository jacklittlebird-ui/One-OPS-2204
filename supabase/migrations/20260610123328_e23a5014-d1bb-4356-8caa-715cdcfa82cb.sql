...
DECLARE
  ...
  ref_date text;
BEGIN
  -- Skip when none of the watched fields actually changed (avoids ping-pong with sync_security_dispatch_to_flight_schedule)
  IF TG_OP = 'UPDATE' AND
  ...
  WHERE d.flight_schedule_id = NEW.id
    AND (
      ...
      OR COALESCE(d.task_sheet_data->>'flight_type','') IS DISTINCT FROM COALESCE(NULLIF(NEW.skd_type, ''), COALESCE(d.task_sheet_data->>'flight_type',''))
