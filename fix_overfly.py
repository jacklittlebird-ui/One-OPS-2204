...
with open('src/pages/OverflySchedule.tsx', 'r') as f:
    lines = f.readlines()
...
for i, line in enumerate(lines):
    if 'const filtered = useMemo(() => {' in line:
        for j in range(i, len(lines)):
            if '}, [data, statusFilter, operatorFilter, dateFrom, dateTo, search]);' in lines[j]:
                lines.insert(j + 1, '  const { pageRows, ...pag } = usePagination(filtered, { resetKey: [statusFilter, operatorFilter, dateFrom, dateTo, search] });\n')
                break
        break

# Replace pageData.map with pageRows.map
for i, line in enumerate(lines):
...
with open('src/pages/OverflySchedule.tsx', 'w') as f:
    f.writelines(lines)
