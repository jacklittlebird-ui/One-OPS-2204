...
type Row = { id: string; service: string; unit: string; price: string; airport: string };

export default function TubePage() {
  const { data, isLoading } = useSupabaseTable<Row>("tube_charges", { orderBy: "created_at", ascending: true });
  const handleExport = () => exportToExcel(data.map(r => ({ Service: r.service, Unit: r.unit, "Price (USD)": r.price, Airport: r.airport })), "Tube Charges", "Link_Tube_Charges.xlsx");

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Building2 size={22} className="text-primary" /> Tube Bridge Charges</h1><p className="text-muted-foreground text-sm mt-1">Jetway / tube bridge usage fees by airport</p></div>
