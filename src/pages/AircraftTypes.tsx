...
const catCfg: Record<string, string> = {
  ...
  WB: "bg-primary/15 text-primary",
  SH: "bg-destructive/15 text-destructive",
  RJ: "bg-warning/15 text-warning",
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {Object.entries(catLabel).map(([k, v]) => (
          <div key={k} className="stat-card"><div className={`stat-card-icon ${catCfg[k]?.replace('/15', '').split(' ')[0]}`}><Plane size={20} /></div><div><div className="text-2xl font-bold text-foreground">{aircraftTypesRef.filter(a => a.category === k).length}</div><div className="text-xs text-muted-foreground">{v}</div></div></div>
        ))}
      </div>
      <div className="bg-card rounded-lg border overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(a => (
                <tr key={a.icao} className="data-table-row">
                  ...
                  <td className="px-4 py-2.5"><span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${catCfg[a.category]}`}>{a.category} — {catLabel[a.category]}</span></td>
