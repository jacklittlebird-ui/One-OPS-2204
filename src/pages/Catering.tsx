import { cateringItems } from "@/data/servicesData";
...
export default function CateringPage() {
  ...
  const handleExport = () => exportToExcel(
    cateringItems.map(r => ({ Item: r.item, Unit: r.unit, "Price (USD)": r.price, Category: r.category })),
    "Catering", "Link_Catering.xlsx"
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><UtensilsCrossed size={22} className="text-primary" /> Catering Services</h1>
          <p className="text-muted-foreground text-sm mt-1">In-flight catering, supplies, and food service pricing</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="stat-card"><div className="stat-card-icon bg-warning"><UtensilsCrossed size={20} /></div><div><div className="text-2xl font-bold text-foreground">{cateringItems.length}</div><div className="text-xs text-muted-foreground">Menu Items</div></div></div>
                ...
                <tbody>
                  {items.map((r, i) => (
                    <tr key={i} className="data-table-row">
                      <td className="px-4 py-2.5 font-semibold text-foreground">{r.item}</td>
