...
const catCfg: Record<string, string> = {
  ...
  Form: "bg-info/15 text-info",
  Checklist: "bg-warning/15 text-warning",
  SOP: "bg-success/15 text-success",
};
...
export default function ManualsAndFormsPage() {
  ...
  const filtered = useMemo(() => {
    let r: ManualForm[] = sampleManualsAndForms;
    if (catFilter !== "All") r = r.filter(m => m.category === catFilter);
    if (search) { const s = search.toLowerCase(); r = r.filter(m => m.title.toLowerCase().includes(s) || m.id.toLowerCase().includes(s)); }
    return r;
  }, [search, catFilter]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="stat-card"><div className="stat-card-icon bg-success"><CheckCircle size={20} /></div><div><div className="text-2xl font-bold text-foreground">{sampleManualsAndForms.filter(m => m.status === "Current").length}</div><div className="text-xs text-muted-foreground">Current</div></div></div>
        <div className="stat-card"><div className="stat-card-icon bg-warning"><Clock size={20} /></div><div><div className="text-2xl font-bold text-foreground">{sampleManualsAndForms.filter(m => m.status === "Under Review").length}</div><div className="text-xs text-muted-foreground">Under Review</div></div></div>
      </div>
      <div className="bg-card rounded-lg border overflow-hidden">
        <div className="p-4 border-b flex flex-wrap items-center gap-3">
          ...
          <select value={catFilter} onChange={e => setCatFilter(e.target.value)} className="text-sm border rounded px-2 py-1.5 bg-card text-foreground">
            <option>All</option><option>Manual</option><option>Form</option><option>Checklist</option><option>SOP</option>
          </select>
        </div>
            ...
            <tbody>
              {filtered.map(m => (
                <tr key={m.id} className="data-table-row">
                  <td className="px-3 py-2.5 font-mono text-xs font-semibold text-primary">{m.id}</td>
                  ...
                  <td className="px-3 py-2.5 text-foreground whitespace-nowrap">{m.lastUpdated}</td>
