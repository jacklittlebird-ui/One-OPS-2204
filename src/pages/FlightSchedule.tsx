...
export default function FlightSchedulePage() {
  ...
  const { data, isLoading, add, update, remove, bulkInsert } = useSupabaseTable<FlightRow>("flight_schedules");
  const [search, setSearch] = useState("");
  const [airlineFilter, setAirlineFilter] = useState("All Airlines");
  const [statusFilter, setStatusFilter] = useState("All Status");
  const [page, setPage] = useState(1);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editRow, setEditRow] = useState<Partial<FlightRow>>({});
  const [showAdd, setShowAdd] = useState(false);
  const [newRow, setNewRow] = useState<Partial<FlightRow>>({ flight_no: "", airline: "", origin: "", destination: "", departure: "", arrival: "", aircraft: "", days: "", status: "Scheduled", terminal: "" });
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(r => r.flight_no.toLowerCase().includes(s) || r.airline.toLowerCase().includes(s) || r.origin.toLowerCase().includes(s) || r.destination.toLowerCase().includes(s));
    }
    return result;
  ...
  const startEdit = (row: FlightRow) => { setEditingId(row.id); setEditRow({ ...row }); };
  ...
  const deleteRow = async (id: string) => { await remove(id); };
  const addRow = async () => {
    if (!newRow.flight_no || !newRow.airline) return;
    await add(newRow as any);
    setShowAdd(false);
    setNewRow({ flight_no: "", airline: "", origin: "", destination: "", departure: "", arrival: "", aircraft: "", days: "", status: "Scheduled", terminal: "" });
  };
  ...
  const handleUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      const rows = json.map((row: any) => ({
        flight_no: row["Flight No"] || row.flight_no || "",
        ...
        arrival: row["Arrival"] || row.arrival || "",
  ...
  const handleExport = () => {
    const ws = XLSX.utils.json_to_sheet(filtered.map(r => ({ "Flight No": r.flight_no, Airline: r.airline, Origin: r.origin, Destination: r.destination, Departure: r.departure, Arrival: r.arrival, Aircraft: r.aircraft, Days: r.days, Terminal: r.terminal, Status: r.status })));
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="stat-card"><div className="stat-card-icon bg-success"><CheckCircle size={20} /></div><div><div className="text-2xl font-bold text-foreground">{airlines.length}</div><div className="text-xs text-muted-foreground">Airlines Operating</div></div></div>
      </div>
        ...
        {showAdd && (
          <div className="p-4 border-b bg-muted">
            <div className="grid grid-cols-11 gap-2 items-end">
              <input placeholder="Flight No" value={newRow.flight_no || ""} onChange={e => setNewRow(p => ({ ...p, flight_no: e.target.value }))} className="text-sm border rounded px-2 py-1.5 bg-card text-foreground" />
              ...
              <select value={newRow.status || "Scheduled"} onChange={e => setNewRow(p => ({ ...p, status: e.target.value }))} className="text-sm border rounded px-2 py-1.5 bg-card text-foreground">
            ...
            <tbody>
              {pageData.length === 0 ? (
                ...
                <tr key={row.id} className="data-table-row">
                  {editingId === row.id ? (
                    <>
                      <td className="px-4 py-2"><input value={editRow.flight_no || ""} onChange={e => setEditRow(p => ({ ...p, flight_no: e.target.value }))} className="text-sm border rounded px-1.5 py-0.5 w-20 bg-card text-foreground" /></td>
                      ...
                      <td className="px-4 py-2">
                        <select value={editRow.status || "Scheduled"} onChange={e => setEditRow(p => ({ ...p, status: e.target.value }))} className="text-sm border rounded px-1.5 py-0.5 bg-card text-foreground">
                          <option>Scheduled</option><option>Delayed</option><option>Cancelled</option><option>Completed</option>
                        </select>
                      </td>
                    ...
                    <>
                      <td className="px-4 py-2.5 text-foreground font-mono font-semibold">{row.flight_no}</td>
