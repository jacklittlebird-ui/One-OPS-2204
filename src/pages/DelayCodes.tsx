...
import { DelayCode, sampleDelayCodes } from "@/data/delayCodesData";
                  ...
                  {editingId === c.id ? (
                    <>
                      ...
                      <td className="px-4 py-2">
                        <select value={editRow.responsible} onChange={e => set("responsible", e.target.value)} className={inp}>
                          <option>Airline</option><option>Airport</option><option>ATC</option><option>Weather</option><option>Handling</option><option>Security</option><option>Other</option>
                        </select>
                      </td>
                      <td className="px-4 py-2">
                        <select value={editRow.impactLevel} onChange={e => set("impactLevel", e.target.value)} className={inp}>
                          <option>Low</option><option>Medium</option><option>High</option>
                        </select>
                      </td>
                      <td className="px-4 py-2"><input type="number" value={editRow.avgMinutes || 0} onChange={e => set("avgMinutes", +e.target.value)} className={inp + " w-16"} /></td>
                      <td className="px-4 py-2 flex gap-1">
                        <button onClick={() => { setCodes(p => p.map(x => x.id === c.id ? { ...x, ...editRow } as DelayCode : x)); setEditingId(null); }} className="toolbar-btn-success text-xs py-1">Save</button>
                    ...
                    <>
                      ...
                      <td className="px-4 py-2.5"><span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${impactColor[c.impactLevel]}`}>{c.impactLevel}</span></td>
                      <td className="px-4 py-2.5 text-foreground">{c.avgMinutes} min</td>
                      <td className="px-4 py-2.5">
                        <div className="flex gap-1.5">
                          <button onClick={() => { setEditingId(c.id); setEditRow({ ...c }); }} className="text-info hover:text-info/80"><Pencil size={13} /></button>
                          <button onClick={() => setCodes(p => p.filter(x => x.id !== c.id))} className="text-destructive hover:text-destructive/80"><Trash2 size={13} /></button>
                        </div>
                      </td>
                    </>
                  )}
